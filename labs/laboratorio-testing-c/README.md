...
> R: deja la memoria vacia, se la llama antes de usar carrito porque si no se sumaria lo de el carrito a la basura que tiene la memoria.
...
> R: falla, esperaba 700, obtuvo 350
...
> R: linea 24, recorre los productos del carrito con un for y los va sumando en la variable total

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: deberia multiplicar el precio por la cantidad de productos
...
> R: La linea 30 y 31 no se ejecutaron porque ningun test unitario llama a esa funcion
...
> R: El unitario verifica una sola funcion de forma aislada y el de integracion varias en conjunto. El unitario detecto primero
...
> R:porque al mirar el codigo se te puede pasar algun error. Los test son automaticos y si cambias algo en el codigo el dia de mañana solo corres el test y ya sabes si hay error
...
> R: no importaba si habia que multiplicarlo porque era solo una unidad, daba lo mismo. En el segundo ya era necesario
